-- Update get_driver_tasks to prioritize tasks by status (using Hebrew enum values)
-- Order: בעבודה (in_progress) first, then בהמתנה (pending) by estimated_start, then הושלמה (completed) last
